...
function HubTabs({ onStartGame, profile, setProfile, openSettings, openAchievements, charData, onEditCharacter }) {
  ...
  return (
    <View style={{ width: LW, height: LH, backgroundColor: '#070615' }}>
      <StatusBar hidden />
      <View style={{ flex: 1, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 4 }}>
        {tab === 'home' && (
...
function BottomNav({ tab, onTab, onStart }) {
  const { width: W, height: H } = useWindowDimensions();
  const LH = Math.min(W, H);
  // Smaller buttons on small landscape phones
  const BTN_H = Math.round(Math.max(36, Math.min(44, LH * 0.105)));
  ...
  const Item = ({ id, label }) => {
    ...
    return (
      ...
      >
        <Text style={{ color: active ? '#e8dcc8' : 'rgba(232,220,200,0.55)', fontFamily: 'monospace', fontSize: 10, letterSpacing: 1 }}>
          {label}
        </Text>
      </TouchableOpacity>
    );
  ...
  return (
    ...
    >
      ...
      >
        <Text style={{ color: '#03020a', fontFamily: 'monospace', fontWeight: '900', fontSize: 13, letterSpacing: 3 }}>
          START
        </Text>
      </TouchableOpacity>
...
function HomeScreen({ profile, charData, onPlay }) {
  ...
  const fallbackTips = [
    'Collect all 4 keys across the floors, then open the exit.',
    'Double-tap a direction to sprint for 20 seconds.',
    'Freeze stops all ghosts — walk over the pickup to auto-collect.',
    'Dark Mode: STAY SILENT. Screaming attracts ghosts.',
    'Shield blocks one ghost hit. Arm it before entering danger zones.',
  ];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rawCoins = await storage.get(COINS_KEY);
        if(rawCoins && alive){ try{ setCoins(parseInt(rawCoins,10)||0); }catch(e){} }
      } catch(e) {}
      try {
        ...
        if(alive) setTipState({ loading:false, text:tip, source:'Game Tip' });
      }
    })();
    return () => { alive = false; };
  }, []);

  const tip = tipState.loading ? 'Loading...' : tipState.text;
  const winRate = (stats.escapes + stats.deaths) > 0 ? Math.round((stats.escapes / (stats.escapes + stats.deaths)) * 100) : 0;

  return (
    <View style={{ flex:1, flexDirection:'row', gap:10 }}>

      {/* LEFT COLUMN — identity + title */}
      <View style={{ width: LW * 0.36, justifyContent:'center', alignItems:'center', gap:8,
        borderRightWidth:1, borderRightColor:'rgba(255,255,255,0.06)', paddingRight:10 }}>

        {/* Avatar */}
        <View style={{
          width:64, height:64, borderRadius:20,
          backgroundColor:'rgba(201,164,76,0.10)',
          borderWidth:1.5, borderColor:'rgba(201,164,76,0.30)',
          justifyContent:'center', alignItems:'center',
        }}>
          {charData ? <ChibiCharacter charData={charData} size={1.3}/> :
            <Text style={{color:'rgba(255,255,255,0.2)',fontFamily:'monospace',fontWeight:'900',fontSize:22}}>?</Text>}
        </View>

        <Text style={{color:'#e8dcc8',fontFamily:'monospace',fontWeight:'900',fontSize:14,letterSpacing:4,textAlign:'center',
          textShadowColor:'rgba(255,0,80,0.4)',textShadowRadius:12}}>
          THE LAST{'\n'}FLOOR
        </Text>

        <Text style={{color:'#d6c8ff',fontFamily:'monospace',fontWeight:'900',fontSize:11,letterSpacing:1,textAlign:'center'}}>
          {name}
        </Text>
        <Text style={{color:'rgba(255,255,255,0.28)',fontFamily:'monospace',fontSize:8,letterSpacing:2,textAlign:'center'}}>
          {stats.escapes>5?'VETERAN':stats.escapes>2?'ESCAPIST':stats.escapes>0?'RUNNER':'NEWCOMER'}
        </Text>

        {/* Coins pill */}
        <View style={{flexDirection:'row',alignItems:'center',gap:6,
          paddingHorizontal:12,paddingVertical:6,borderRadius:20,
          backgroundColor:'rgba(201,164,76,0.10)',borderWidth:1,borderColor:'rgba(201,164,76,0.30)'}}>
          <Text style={{fontSize:13}}>🪙</Text>
          <Text style={{color:'#c9a44c',fontFamily:'monospace',fontWeight:'900',fontSize:14}}>{coins}</Text>
        </View>
      </View>
      ...
      <View style={{ flex:1, justifyContent:'center', gap:8 }}>
        ...
        <View style={{flexDirection:'row',gap:6}}>
          {[
            {label:'WIN%',  value:`${winRate}%`,              color:'#5ecfa0'},
            ...
            <View key={s.label} style={{flex:1,backgroundColor:'rgba(0,0,0,0.35)',borderRadius:10,
              padding:8,borderWidth:1,borderColor:'rgba(255,255,255,0.07)',alignItems:'center',gap:2}}>
              <Text style={{color:s.color,fontFamily:'monospace',fontWeight:'900',fontSize:13}}>{s.value}</Text>
              <Text style={{color:'rgba(255,255,255,0.30)',fontFamily:'monospace',fontSize:7,letterSpacing:1}}>{s.label}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{borderRadius:12,padding:12,
          ...
          <Text style={{color:'rgba(201,164,76,0.80)',fontFamily:'monospace',fontWeight:'900',fontSize:9,letterSpacing:3,marginBottom:5}}>
            💡 {tipState.source?.toUpperCase() || 'TIP'}
          </Text>
          <Text style={{color:'rgba(255,255,255,0.65)',fontFamily:'monospace',fontSize:10,lineHeight:16}} numberOfLines={3}>
            {tip}
          </Text>
        </View>
        ...
        <View style={{flexDirection:'row',alignItems:'center',gap:8,
          padding:10,borderRadius:12,backgroundColor:'rgba(201,164,76,0.06)',
          borderWidth:1,borderColor:'rgba(201,164,76,0.20)'}}>
          <Text style={{fontSize:16}}>🎯</Text>
          <Text style={{flex:1,color:'rgba(255,255,255,0.50)',fontFamily:'monospace',fontSize:9,lineHeight:14}}>
            {'4 floors · 4 keys · 1 exit\nCollect all keys → reach elevator → escape'}
          </Text>
        </View>
      </View>
    </View>
...
function RulesScreen() {
  ...
  return (
    <ScrollView style={{flex:1}} contentContainerStyle={{paddingBottom:40,paddingHorizontal:14}}
      ...
      <Section label="COINS"/>
      <Rule sym="🪙" symColor="#c9a44c" title="Earning Coins" body="Complete a floor → earn coins equal to that floor number (Floor 1 = 1 coin, Floor 2 = 2 coins, up to 4). If you die, you still earn 1 coin for each floor you already completed that run. Push a ghost with PUSH → bonus +1 coin instantly!"/>
      <Rule sym="WIN" symColor="#ffdd66" title="Win Bonus" body="Escape all 4 floors and you keep the full 10 coins (1+2+3+4). Coins accumulate across all your runs and are shown on the Home screen."/>
...
function ProfileScreen({ profile, setProfile, charData, onEditCharacter, onPlay, openSettings, openAchievements }) {
  ...
  const Btn = ({label, onPress, color='#c9a44c', bg='rgba(201,164,76,0.10)', border='rgba(201,164,76,0.35)', icon=''}) => (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}
      style={{flexDirection:'row', alignItems:'center', gap:8, paddingVertical:10, paddingHorizontal:12,
        borderRadius:10, backgroundColor:bg, borderWidth:1.5, borderColor:border}}>
      {!!icon && <Text style={{fontSize:13}}>{icon}</Text>}
      <Text style={{color, fontFamily:'monospace', fontWeight:'900', fontSize:10, letterSpacing:1, flex:1}}>{label}</Text>
  ...
  return (
    <View style={{flex:1, flexDirection:'row', gap:10}}>
      ...
      <View style={{width: LW * 0.32, justifyContent:'center', alignItems:'center', gap:8,
        borderRightWidth:1, borderRightColor:'rgba(255,255,255,0.06)', paddingRight:10}}>

        <View style={{width:70, height:70, borderRadius:22, backgroundColor:'rgba(0,0,0,0.4)',
          borderWidth:2, borderColor:'rgba(201,164,76,0.35)', justifyContent:'center', alignItems:'center'}}>
          {charData ? <ChibiCharacter charData={charData} size={1.35}/> :
            <Text style={{color:'rgba(255,255,255,0.2)', fontSize:22}}>?</Text>}
        </View>

        {editing ? (
          <View style={{width:'100%', gap:6}}>
            <TextInput value={name} onChangeText={setName} autoFocus maxLength={20}
              style={{color:'#e8dcc8', fontFamily:'monospace', fontWeight:'900', fontSize:14, textAlign:'center',
                backgroundColor:'rgba(255,255,255,0.06)', borderRadius:8, paddingHorizontal:10, paddingVertical:6,
                borderWidth:1.5, borderColor:'#c9a44c'}}/>
            <View style={{flexDirection:'row', gap:6}}>
              <TouchableOpacity onPress={save} style={{flex:1, backgroundColor:'#c9a44c', borderRadius:8, paddingVertical:7, alignItems:'center'}}>
                <Text style={{color:'#000', fontFamily:'monospace', fontWeight:'900', fontSize:10}}>SAVE</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={()=>setEditing(false)} style={{width:34, backgroundColor:'rgba(255,255,255,0.07)', borderRadius:8, paddingVertical:7, alignItems:'center'}}>
                <Text style={{color:'rgba(255,255,255,0.6)', fontFamily:'monospace', fontSize:10}}>✕</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : (
          <TouchableOpacity onPress={()=>setEditing(true)} style={{alignItems:'center', gap:4}}>
            <Text style={{color:'#e8dcc8', fontFamily:'monospace', fontWeight:'900', fontSize:15, letterSpacing:1}}>{name||'Human'}</Text>
            <View style={{borderWidth:1, borderColor:'rgba(201,164,76,0.4)', borderRadius:5, paddingHorizontal:7, paddingVertical:2}}>
              <Text style={{color:'rgba(201,164,76,0.7)', fontFamily:'monospace', fontSize:8, letterSpacing:2}}>TAP TO EDIT</Text>
            </View>
          </TouchableOpacity>
        )}
        {!!msg && <Text style={{color:'#19C37D', fontFamily:'monospace', fontSize:9, letterSpacing:2}}>{msg}</Text>}
        <Text style={{color:'rgba(255,255,255,0.22)', fontFamily:'monospace', fontSize:8, letterSpacing:2, textAlign:'center'}}>
      ...
      <View style={{flex:1, justifyContent:'center', gap:8}}>
        ...
        <View style={{flexDirection:'row', gap:5}}>
          {[
            {label:'ESCAPES', value:stats.escapes, color:'#c9a44c'},
            {label:'DEATHS',  value:stats.deaths,  color:'#ff6666'},
            {label:'WIN%',    value:`${winRate}%`,  color:'#19C37D'},
            {label:'BEST',    value:msToClock(stats.bestEscapeMs), color:'#88ddff'},
          ].map(s=>(
            <View key={s.label} style={{flex:1, backgroundColor:'rgba(0,0,0,0.35)', borderRadius:10,
              padding:8, borderWidth:1, borderColor:'rgba(255,255,255,0.07)', alignItems:'center', gap:2}}>
        ...
        {/* Action buttons */}
        <View style={{gap:6}}>
          <Btn label="EDIT CHARACTER" icon="🎭" onPress={onEditCharacter}
            color='#c9a44c' bg='rgba(201,164,76,0.08)' border='rgba(201,164,76,0.35)'/>
          <Btn label="ACHIEVEMENTS" icon="🏆" onPress={openAchievements}
            color='#b0a0ff' bg='rgba(100,80,255,0.06)' border='rgba(100,80,255,0.25)'/>
          <Btn label="SETTINGS" icon="⚙️" onPress={openSettings}
            color='rgba(255,255,255,0.70)' bg='rgba(255,255,255,0.03)' border='rgba(255,255,255,0.12)'/>
        </View>
      </View>
    </View>
  );
...
function DeadScreen({charData,onRetry,onMenu,floorNum=1,totalCoins=0,continueCost=15,onContinuePaid}){
  const fade   = useRef(new Animated.Value(0)).current;
  ...
  const handleContinue=()=>{
    if(!canAfford){ Alert.alert('Not enough coins',`Need ${continueCost}🪙. You have ${totalCoins}🪙.`); return; }
    Alert.alert('Continue?',`Pay ${continueCost}🪙 to revive on Floor ${floorNum}?\nRemaining: ${totalCoins-continueCost}🪙\nNext continue costs: ${continueCost*2}🪙`,
      [{text:'Cancel',style:'cancel'},{text:`Pay ${continueCost}🪙`,onPress:()=>onContinuePaid&&onContinuePaid()}]);
  };
  ...
  return(
    <Animated.View style={{width:LW,height:LH,backgroundColor:'#060004',flexDirection:'row',opacity:fade}}>
      ...
      <View style={{flex:1,justifyContent:'center',paddingHorizontal:20,gap:10}}>
        <View style={{borderRadius:12,padding:12,
          backgroundColor:'rgba(255,255,255,0.03)',borderWidth:1,borderColor:'rgba(255,255,255,0.08)'}}>
          <Text style={{color:'rgba(201,164,76,0.7)',fontFamily:'monospace',fontSize:8,letterSpacing:3,marginBottom:5}}>SURVIVAL TIP</Text>
          <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:10,lineHeight:16}}>{tip}</Text>
        </View>
        ...
        <TouchableOpacity onPress={handleContinue} activeOpacity={0.85}
          style={{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,
            ...
            backgroundColor:canAfford?'rgba(201,164,76,0.18)':'rgba(201,164,76,0.04)',
            borderWidth:1.5,borderColor:canAfford?'#c9a44c':'rgba(201,164,76,0.25)'}}>
          <Text style={{fontSize:14,opacity:canAfford?1:0.4}}>🪙</Text>
          <Text style={{color:canAfford?'#c9a44c':'rgba(201,164,76,0.35)',fontFamily:'monospace',fontWeight:'900',fontSize:12,letterSpacing:1}}>
        ...
        <View style={{flexDirection:'row',gap:8}}>
          <TouchableOpacity onPress={onRetry} activeOpacity={0.85}
            style={{flex:1,paddingVertical:12,borderRadius:10,
              backgroundColor:'rgba(200,20,40,0.12)',borderWidth:1.5,borderColor:'#cc2233',alignItems:'center'}}>
            <Text style={{color:'#ff4455',fontFamily:'monospace',fontWeight:'900',fontSize:11,letterSpacing:2}}>
              RETRY F{floorNum}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onMenu} activeOpacity={0.85}
            ...
            <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:11,letterSpacing:1}}>
              ⌂ HOME
            </Text>
          </TouchableOpacity>
        </View>
...
function WinScreen({charData,onPlay,onMenu,durMs=0,bestEscapeMs=0,brokeRecord=false,escapes=0}){
  ...
  const tip=tips[(floorNum-1)%tips.length];
  // M4.2: Check if player can afford to continue
  const canAfford=totalCoins>=continueCost;
  ...
  const handleContinue=()=>{
    if(!canAfford){
      Alert.alert(
        'Not enough coins',
        `You need ${continueCost}🪙 to continue.\nYou have ${totalCoins}🪙.\n\nCollect more coins by surviving floors and picking up coins in the maze.`
      );
      return;
    }
    Alert.alert(
      'Continue?',
      `Pay ${continueCost}🪙 to revive on Floor ${floorNum}?\n\nYou have: ${totalCoins}🪙\nRemaining after: ${totalCoins-continueCost}🪙\n\nNext continue will cost ${continueCost*2}🪙`,
      [
        {text:'Cancel',style:'cancel'},
        {text:`Pay ${continueCost}🪙`,style:'default',onPress:()=>onContinuePaid&&onContinuePaid()},
      ]
    );
  };
  ...
  return(
    <View style={{flex:1,backgroundColor:'#060004'}}>
      <StatusBar hidden/>
      <View style={{position:'absolute',left:0,right:0,top:0,bottom:0,backgroundColor:'rgba(180,0,20,0.08)'}} pointerEvents="none"/>
      <View style={{position:'absolute',top:0,left:0,right:0,height:3,backgroundColor:'#aa0015'}}/>
      ...
      <Animated.View style={{flex:1,opacity:fade,transform:[{translateY:slideY}],
        ...
        <View style={{width:'60%',height:1,backgroundColor:'rgba(200,0,20,0.3)',marginBottom:16}}/>

        {/* M4.2: Coin balance display */}
        <View style={{flexDirection:'row',alignItems:'center',gap:8,marginBottom:16,
          paddingHorizontal:16,paddingVertical:8,borderRadius:12,
          backgroundColor:'rgba(201,164,76,0.08)',borderWidth:1,borderColor:'rgba(201,164,76,0.25)'}}>
          <Text style={{fontSize:18}}>🪙</Text>
          <Text style={{color:'#c9a44c',fontFamily:'monospace',fontWeight:'900',fontSize:16}}>{totalCoins}</Text>
          <Text style={{color:'rgba(255,255,255,0.35)',fontFamily:'monospace',fontSize:10,letterSpacing:2}}>COINS</Text>
        </View>
        ...
        <View style={{
          ...
          maxWidth:340,width:'100%',marginBottom:20,
        }}>
        ...
        <View style={{gap:10,width:'100%',maxWidth:340}}>
          {/* M4.2: Continue by paying coins — cost doubles each death */}
          <TouchableOpacity onPress={handleContinue} activeOpacity={0.85}
            style={{
              borderWidth:1.5,
              borderColor:canAfford?'#c9a44c':'rgba(201,164,76,0.30)',
              paddingVertical:16,borderRadius:12,
              backgroundColor:canAfford?'rgba(201,164,76,0.16)':'rgba(201,164,76,0.05)',
              alignItems:'center',flexDirection:'row',justifyContent:'center',gap:8,
            }}>
            <Text style={{fontSize:14,opacity:canAfford?1:0.4}}>🪙</Text>
            <Text style={{
              color:canAfford?'#c9a44c':'rgba(201,164,76,0.35)',
              fontFamily:'monospace',fontWeight:'900',letterSpacing:2,fontSize:13,
            }}>
              CONTINUE  {continueCost}🪙
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={onRetry} activeOpacity={0.85}
            style={{
              ...
              backgroundColor:'rgba(200,20,40,0.10)',alignItems:'center',
            }}>
            <Text style={{color:'#ff4455',fontFamily:'monospace',fontWeight:'900',letterSpacing:3,fontSize:12}}>
...
function GameCore({ navigation }){
  ...
  const HUD_H      = Math.round(Math.max(36, Math.min(44, SH * 0.095)));
  const HINT_H     = Math.round(Math.max(18, Math.min(22, SH * 0.048)));
  // ACT_H: d-pad button size — responsive to screen height
  const ACT_H      = Math.round(Math.max(26, Math.min(34, SH * 0.082)));
  const SAFE_BOT   = Platform.OS === 'ios' ? 10 : 2;
  const CONTROLS_H = ACT_H * 3 + SAFE_BOT + 8;
...
const activateFreeze = useCallback(()=>{
  ...
  const finishRun = useCallback(async(outcome, extra={})=>{
    ...
    if(outcome === 'win'){
      ...
      // Award coin for floor 4 completion (floors 1-3 already awarded during elevator rides)
      floorsCompletedThisRunRef.current += 1;
      const floor4Coin = floorsCompletedThisRunRef.current; // should be 4
      const winCoinGain = floor4Coin;
      ...
      // Award 1 coin per floor completed before dying
  ...
  if(screen==='dead')return(
    <DeadScreen
      ...
      continueCost={continueCostRef.current}
      onContinuePaid={async()=>{
        // M4.2: Deduct coins, double next cost, revive player on same floor
      ...
      onRetry={()=>{
        // Restart from SAME floor, reset only current floor state
        const f=floorsRef.current[floorIndex];
        enemiesRef.current=f.enemies.map(e=>({...e,patrolIndex:0,frozenUntil:0}));
        playerRef.current={x:f.spawn.x,y:f.spawn.y};
        setPlayer({x:f.spawn.x,y:f.spawn.y});
  ...
  return(
    <View style={{width:SW, height:SH, backgroundColor:theme.bg, overflow:'hidden', position:'relative'}}>
      ...
      <View style={{
        ...
        {/* ACTION BUTTONS — 2 rows of compact buttons, fits all phone sizes */}
        <View style={{gap:3,flexShrink:1}}>
          {/* Row 1: FREEZE  PUSH  SHIELD */}
          <View style={{flexDirection:'row',gap:4}}>
            <TouchableOpacity
              disabled={pauseOpen||elevatorBusy||freezeActive}
              style={{paddingHorizontal:8,height:ACT_H,borderRadius:7,
              ...
              <Text style={{color:inv.freezeCharges>0&&!freezeActive?'#88ddff':freezeActive?'#55ccff':'rgba(100,180,220,0.40)',fontWeight:'900',fontSize:9,fontFamily:'monospace'}}>
            ...
            <TouchableOpacity
              ...
              onPress={pushGhost}>
              <Text style={{color:ghostIsClose&&inv.pushCharges>0?'#ff8855':'rgba(255,120,80,0.38)',fontWeight:'900',fontSize:9,fontFamily:'monospace'}}>
                💥 {inv.pushCharges}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Text style={{color:inv.shieldCharges>0?theme.accent:'rgba(255,255,255,0.28)',fontWeight:'900',fontSize:9,fontFamily:'monospace'}}>
                🛡 {inv.shieldCharges}{inv.shieldArmed?'✓':''}
              </Text>
            </TouchableOpacity>
          </View>

          {/* Row 2: HOLD/KEY  ELEVATOR */}
          <View style={{flexDirection:'row',gap:4}}>
            <TouchableOpacity
              disabled={!canCapture||pauseOpen||elevatorBusy}
              style={{flex:1,paddingHorizontal:8,height:ACT_H,borderRadius:7,
              ...
              <Text style={{color:canCapture?'white':'rgba(255,255,255,0.35)',fontWeight:'900',fontSize:9,fontFamily:'monospace'}} numberOfLines={1}>
                {captureLabel}
              </Text>
            </TouchableOpacity>
              ...
              Alert.alert(
                ...
                [
                  ...
                  {text:'DELETE EVERYTHING',style:'destructive',onPress:async()=>{
                    try{
                      // M4.2: CRUD Delete — wipe every persisted key from device storage
                      if(AsyncStorage){
                        try{
                        ...
                        // In-memory fallback store clear
                        for(const key of ALL_STORAGE_KEYS){
                      ...
                      Alert.alert(
                        'Done',
                        'All data has been permanently deleted. The app will now reset.',
                        [{text:'OK',onPress:()=>{
                          try{navigation.replace('Game');}catch(e){
                            try{navigation.navigate('Game');}catch(ee){}
                          }
                        }}]
                      );
                    }catch(err){
                      Alert.alert('Error',`Could not delete data: ${err?.message||'Unknown error'}`);
                    }
                  }},
                ]
              );
